...
import { urlGetTopic, urlInsertTopic, urlUpdateTopic } from "../url"

const Them_suaChuDe = (props) => {
    //xử lý redux
    ...
    useEffect(() => {
        if (props.iDAction) {
            ...
            const fetchGetAccount = fetch(`${urlGetTopic}?id=${props.iDAction}`, {
                method: 'GET',
                headers: {
    ...
    const handleSubmit = (e) => {
        ...
        if (!dataReq.TenChuDe
            ...
            dispatch({ type: 'SET_LOADING', payload: true })
            const data = {
                MaChuDe: dataReq.MaChuDe,
                TenChuDe: dataReq.TenChuDe,
                MoTa: dataReq.MoTa
            };
            if (props.isInsert === true) {
                fetch(urlInsertTopic, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'ss': getCookie('ss'),
                    ...
                    .then(data => {
                        ...
                        props.setdataUser({ ...props.dataUser, sortBy: 'MaChuDe', sortOrder: 'desc' })
                    })
                    .catch(error => {
                        dispatch({ type: 'SET_LOADING', payload: false })
                        if (error instanceof TypeError) {
                ...
                <div className="conten-modal">
                    <div>
                        <div className="bg-light px-4 py-3">
                            <h4 id='tieudepop'>Thông Tin Chủ Đề<span style={{ color: 'blue' }}>ㅤ{props.iDAction}</span></h4>
                            <form onSubmit={handleSubmit}
                                ...
                                <div className="">
                                    <div className="form-group">
                                        <label>Tên Chủ Đề {batBuocNhap}</label>
                                        <input
                                            type="text"
                                            className="form-control"
                                            value={dataReq.TenChuDe}
                                            onChange={(event) => {
                                                setDataReq({
                                                    ...dataReq,
                                                    TenChuDe: event.target.value
                                                });
                                            }}
                                        />
                                    </div>
