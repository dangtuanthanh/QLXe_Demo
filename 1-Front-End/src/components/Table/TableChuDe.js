...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.MaChuDe.toString());
        ...
        <table class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    /></th>
                    {/* <th style={{ padding: 8, textAlign:'center'}} onClick={() => handleClickSort('MaChuDe')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Mã Nhóm Loại Xe</th> */}
                    <th style={{ padding: 8 }} onClick={() => handleClickSort('TenChuDe')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Tên Chủ Đề </th>
                    <th style={{ padding: 8,textAlign:'center' }} onClick={() => handleClickSort('SoLuongBaiDang')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Số Lượng Bài Đăng </th>
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        <tr style={{ 'textAlign': 'center' }} id='trdata' key={dulieu.MaChuDe} onClick={() => {
                            props.setIsInsert(false)
                            props.setIDAction(dulieu.MaChuDe)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.MaChuDe}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.MaChuDe.toString())}
                                    onClick={(e) => e.stopPropagation()}
                                    onChange={handleCheckboxChange}
                                />

                            </td>
                            {/* <td >{dulieu.MaChuDe}</td> */}
                            <td style={{ textAlign: 'left' }}>
                                {
                                    dulieu.TenChuDe ?
                                        dulieu.TenChuDe.length > 30 ?
                                            dulieu.TenChuDe.slice(0, 30) + '...' :
                                            dulieu.TenChuDe
                                        : ''
                                }
                            </td>
                            <td>
                                {dulieu.SoLuongBaiDang}
                            </td>
                            <td style={{ textAlign: 'left' }}>
                                {
                                    dulieu.MoTa ?
                            ...
                            <td>
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.TenChuDe}`,
                                        () => { props.deleteData(dulieu.MaChuDe) }
                                    )
