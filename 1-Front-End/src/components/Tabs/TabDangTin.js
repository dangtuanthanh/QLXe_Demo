...
import { urlGetPost, urlDeletePost } from "../url";
import Pagination from "../Pagination";
import ItemsPerPage from "../ItemsPerPage";
import TableDangTin from "../Table/TableDangTin";
import Them_suaDangTin from "../Popup/them_suaDangTin";
function TabDangTin(props) {
    //xử lý redux
    ...
    //         ...dataUser,
    //         page: 1,
    //         search: 'DangTin đang hoạt động',
    //         searchBy: 'MoTaTinhTrangDangTin'
    //     });
    // };
    // sửa hàng loạt
    ...
    const TaiDuLieu = () => {
        dispatch({ type: 'SET_LOADING', payload: true })
        fetch(`${urlGetPost}?page=${dataUser.page}&limit=${dataUser.limit}&sortBy=${dataUser.sortBy}&sortOrder=${dataUser.sortOrder}&search=${dataUser.search}&searchBy=${dataUser.searchBy}&searchExact=${dataUser.searchExact}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'ss': getCookie('ss'),
        ...
        <div>
            <div class="card" style={{ minHeight: '92vh', position: 'relative' }}>
                <div class="card-header pb-0">
                    <h2 onClick={handleToggleButtonFunction}> Quản Lý Đăng Tin {props.isMobile && <button type="button"  className="btn btn-link btn-sm mb-0 " style={{ width: '100px', float: 'right' }}><FontAwesomeIcon icon={showButtonFunction ? faArrowUp : faArrowDown} /></button>}</h2>
                        ...
                        <div>
                                ...
                                selectedIds.length == 0
                                    ...
                                    : <div style={{ 'display': "inline-block", float: 'left' }}>
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                openPopupAlert(
                                                    `Bạn có chắc chắn muốn xoá các lựa chọn này`,
                            ...
                            <div style={{ 'display': "inline-block", float: 'right' }}>
                                ...
                                <select class="form-select-sm" value={dataUser.searchBy} onChange={handleSearchBy}>
                                    <option value="TieuDe">Tìm theo Tiêu Đề</option>
                                    <option value="NoiDung">Tìm theo Nội Dung</option>
                                    <option value="NgayDang">Tìm theo Ngày Đăng</option>
                                    <option value="TenChuDe">Tìm theo Chủ Đề</option>
                                    <option value="TenThanhVien">Tìm theo Thành Viên</option>
                                </select>
                                ㅤ
                        ...
                        <div style={{
                            ...
                            <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', width: '70%' }}>
                                <div style={{ marginRight: '2rem' }}>
                                    ...
                                    <label style={{ borderTop: '1px solid black', color: 'darkgray' }} >Đang hiển thị: {duLieuHienThi.length}/{dataRes.totalItems} | Sắp xếp{dataRes.sortBy === "NgayDang" ?
