...
import { urlGetTopic, urlDeleteTopic } from "../url";
import Pagination from "../Pagination";
import ItemsPerPage from "../ItemsPerPage";
import TableChuDe from "../Table/TableChuDe";
import Them_suaChuDe from "../Popup/them_suaChuDe";
function TabChuDe(props) {
    //xử lý redux
...
const handleToggleButtonFunction = () => {
    ...
    const TaiDuLieu = () => {
        dispatch({ type: 'SET_LOADING', payload: true })
        fetch(`${urlGetTopic}?page=${dataUser.page}&limit=${dataUser.limit}&sortBy=${dataUser.sortBy}&sortOrder=${dataUser.sortOrder}&search=${dataUser.search}&searchBy=${dataUser.searchBy}&searchExact=${dataUser.searchExact}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'ss': getCookie('ss'),
        ...
        <div>
            <div class="card" style={{ minHeight: '92vh', position: 'relative' }}>
                <div class="card-header pb-0">
                    <h2 onClick={handleToggleButtonFunction}> Quản Lý Chủ Đề{props.isMobile &&<button type="button"  className="btn btn-link btn-sm mb-0 " style={{ width: '100px', float: 'right' }}><FontAwesomeIcon icon={showButtonFunction?faArrowUp :faArrowDown} /></button>}</h2>
                    ...
                    <div>
                        ...
                        <div style={{ 'display': "inline-block", float: 'right' }}>
                            ...
                            <select class="form-select-sm" value={dataUser.searchBy} onChange={handleSearchBy}>
                                <option value="MaChuDe">Tìm theo Mã Chủ Đề</option>
                                <option value="TenChuDe">Tìm theo Tên Chủ Đề</option>
                            </select>
                            ㅤ
